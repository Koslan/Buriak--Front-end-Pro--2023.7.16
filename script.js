...
function renderCategories() {
    ...
    const allButton = document.querySelector(
        '#category-list button[data-category="all"]'
    );
    if (allButton) {
...
function buyBook(id) {
    ...
    if (book) {
        ...
        form.addEventListener("submit", function (event) {
            ...
            <p>Ви замовили ${quantity} примірників "${book.title}" від ${book.author
                } для доставки в ${city}, поштове відділення ${postOffice}.</p>
            <p>Вибраний спосіб оплати: ${payment === "cod" ? "Накладений платіж" : "Оплата карткою"
                }.</p>
